...
/* Remarks:
(a) Reference for extracting hours and minutes information from a time value (DATEPART()): https://learn.microsoft.com/en-us/sql/t-sql/functions/datepart-transact-sql?view=sql-server-ver16
(b) Reference for adding time values together (DATEADD()): https://learn.microsoft.com/en-us/sql/t-sql/functions/dateadd-transact-sql?view=sql-server-ver16
(c) Reference for obtaining current date value (GETDATE() and CONVERT()): https://learn.microsoft.com/en-us/sql/t-sql/functions/getdate-transact-sql?view=sql-server-ver16
(d) MySQL uses different function names for adding time and getting current date */
CREATE VIEW Future_appointments AS
SELECT Date, Time as StartTime, DATEADD(hh, DATEPART(hh, Duration), DATEADD(mi, (DATEPART(mi, Duration)), Time)) AS EndTime, D.Name AS Dentist, P.Name as Patient
...
/* Remarks:
(a) Need to replace NULL values with 0: https://learn.microsoft.com/en-us/sql/t-sql/functions/isnull-transact-sql?view=sql-server-ver16
(b) MySQL uses different function names for converting null value and getting current date */
SELECT PMT.AID, SUM(ISNULL(PMT.PolicyAmount, 0) + ISNULL(PMT.PatientAmount, 0)) AS TotalPaymentMade
...
/* Remarks:
(a) MySQL uses different function names for converting null value and getting current date */
...
FROM (SELECT A.AID, SUM(S.Cost) AS TotalCost
	...
	(SELECT PMT.AID, SUM(ISNULL(PMT.PolicyAmount, 0) + ISNULL(PMT.PatientAmount, 0)) AS TotalPaymentMade
		FROM Payment PMT, Appointment A
		WHERE A.Date < CONVERT(date, GETDATE()) AND PMT.AID = A.AID
		GROUP BY PMT.AID
		UNION
		SELECT AID, ISNULL(NULL, 0) AS TotalPaymentMade
		FROM Appointment
		WHERE AID NOT IN (SELECT DISTINCT AID FROM Payment)
			AND Date < CONVERT(date, GETDATE())) total_paid,
	Appointment A,
    Patient P
...
(Note: Only consider past appointments i.e. before today) */
/* Remarks:
(a) The GROUP BY clause can only use the column selected e.g. P.Name, but not others e.g. P.PID.
Otherwise, MS SQL Server throws error Msg 8120; although this error does not happen in MySQL.
While the result here is OKAY, there may be problem when two patients have the same name;
therefore, it may be wiser to display the PID as well to ensure uniqueness of each row. */
...
/* Remarks:
(a) MySQL uses different function name for converting null value
(b) The GROUP BY clause can only use D.Name to avoid error Msg 8120 same as the one in query 10. */
...
(b) The approach of calculating the total duration is different from that used in MySQL,
